...
public class Statistic {
	public static void main(String[] args) {
		...
		//使用此数据结构以确保存储结果和插入顺序一致
		HashSet<Student> students = new LinkedHashSet<>(110);
		init(students);
		// 存放相同姓名
		HashMap<String, HashSet<Student>> names = new HashMap<>();

		String fname;
		for (Student s : students) {
			fname = s.getName().substring(0, 1);
			if (names.containsKey(fname)) {
				names.get(fname).add(s);
			} else {
				HashSet<Student> hashS = new LinkedHashSet<>();
				hashS.add(s);
				names.put(fname, hashS);
			}
		}
		names.entrySet().stream().sorted((f,s)->{
			return Integer.compare(f.getValue().size(),s.getValue().size());
		}).forEach((k) -> {
			System.out.println(k.getKey().toString() + "\t" + k.getValue().size());
...
//		print(names);
	...
	public static void print(HashMap<String, HashSet<Student>> names) {
		names.entrySet().stream().sorted((f, s) -> {
			int temp = f.getValue().size() - s.getValue().size();
			if (temp > 0) {
				return 1;
			} else if (temp == 0) {
				return 0;
